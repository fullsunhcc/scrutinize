...
def main():
    ...
    task_input = "Command= Give me the steps to place the blocks on their corresponding plates by color."
    ...
    while True:
        instruction = "The robot task failed. Give me the steps to recover it."
        action_step, object_relation = llm.task_planning(instruction, wrong_result, fail_reason, task_input, llm_object_list)
        ...
        for i in range(len(motions)):
            ...
            task_success, wrong_result, fail_reason = failure.failure_detection(relations, object_list, entities, graph, i, Task_GTSGG)

            if not task_success:
                print("Sub-Task failed. Replanning...")
                instruction = "The robot task failed. Give me the steps to recover it."
                break  # Replan if failure occurs
